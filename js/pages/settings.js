...
const FONTS=['Press Start 2P','Courier New','Inter','Atkinson Hyperlegible','Nunito','VT323','IBM Plex Mono'];

export default function renderSettings(root){
  const s=load(); s.user ??= {name:'You'}; s.theme ??= 'default'; s.font ??= 'Press Start 2P';
  s.music ??= {}; s.toddler ??= !!s.toddler;
  save(s);
  root.innerHTML=`
    <h2>Settings</h2>
    <div class="panel">
      ...
      <label><input type="checkbox" id="toddlermode" ${s.toddler?'checked':''}/> Toddler Mode (show Toddler Hub)</label>
  ...
  document.getElementById('font').onchange=e=>{ s.font=e.target.value; save(s); document.documentElement.style.setProperty('--pix', s.font+', system-ui, sans-serif'); };
