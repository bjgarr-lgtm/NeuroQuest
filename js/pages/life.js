...
// Bridge-aware +1 gold (NQ -> fallback to local)
function awardGold1(){
  try{
...
<button id="mealMode" class="secondary">Weekly</button>`;
  ...
  function mealInput(key){
    const inp=document.createElement('input');
    inp.placeholder=key.split('_')[1];
    inp.value=s.meals[key]||'';
    inp.onfocus=()=>{ if(!s.meals[key]) inp.value=''; inp.select(); };
    inp.onchange=()=>{
      const before=s.meals[key]||'';
      s.meals[key]=inp.value.trim();
      save(s);
      // reward only when newly filled (empty -> has text)
      if(!before && s.meals[key]){
        awardGold1();
        try{ crownDrop(); }catch{}
        try{ confetti(); }catch{}
      }
  ...
  let mode='day';
  renderDay('SUN');
  document.getElementById('mealDay').onchange=(e)=>{ const d=e.target.value; if(mode==='day') renderDay(d); };
