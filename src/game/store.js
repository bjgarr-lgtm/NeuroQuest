...
function dealQuests(count=6, seed=Math.random()) {
  const seen = new Set(); const out = [];
  while (out.length < count && seen.size < POOL.length) {
    const q = POOL[rand(POOL.length)];
    if (!seen.has(q.id)) { seen.add(q.id); out.push({ id:q.id, title:q.title, cat:q.cat, reward:{ xp:q.xp, coins:q.coins } }); }
  }
  return out;
}
...
const initial = {
  ...
  summary: null,         // last computed day summary
  history: [],           // array of summaries
};
...
function reducer(state, action) {
  switch(action.type) {
    case 'SET_PARTY':
      return { ...state, hero: action.hero, companion: action.companion };
    case 'START_DAY': {
    ...
    case 'LOCK_IN': {
      ...
      state.quests.forEach(q => {
        ...
        byCat[k].total++;
        if (state.completed[q.id]) byCat[k].done++;
      });
      const summary = {
        date: state.date,
        day: state.day,
        total, done, gainedXP, gainedCoins, byCat
      };
      return { ...state, summary };
    }
    case 'END_DAY': {
      const startNext = !!action.startNext;
      const afterLock = reducer(state, { type:'LOCK_IN' });
      const history = [...afterLock.history, afterLock.summary];
      const base = { ...afterLock, history, completed:{}, quests:[], summary:null };
      return startNext
        ? reducer(base, { type:'START_DAY' })
        : base;
    }
...
function insightsFrom(history) {
  ...
  tips.push(`Your 7-day average completion is ${avgDone}%.`);
  if (topCat) tips.push(`Best category today: ${topCat} (${topPct}%). Double down on what’s working.`);
  if (last.gainedXP >= 100) tips.push('Big XP day! Consider a rest/maintenance day tomorrow.');
  if (last.gainedXP < 40) tips.push('Light XP day. Try 1–2 quick wins early tomorrow.');
  return tips;
}
...
export function useGame(){ const ctx = useContext(Ctx); if(!ctx) throw new Error('useGame must be used inside GameProvider'); return ctx; }
