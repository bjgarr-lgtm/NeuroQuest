...
export default function QuestBoard({ navigation }) {
  ...
  const doneCount = useMemo(()=> Object.keys(state.completed || {}).length, [state.completed]);
  ...
  const renderItem = ({ item:q }) => (
    <View style={[styles.card, isDone(q.id) && styles.done]}>
      ...
      {!isDone(q.id)
        ? (<AnimPressable style={styles.btn} onPress={()=>complete(q)}><Text style={styles.btnText}>Complete</Text></AnimPressable>)
  ...
  const onContinue = () => {
    try { actions?.lockInDay?.(); } catch {}
    navigation.navigate('EndDay'); // new summary screen
  };
  ...
  return (
    <View style={styles.wrap}>
      ...
      <NextBar
        ...
        onNext={onContinue}
        nextLabel={`Continue (${doneCount}/${data.length}) →`}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  check:{ color:'#c9cbe0' },
  toast:{ position:'absolute', bottom:72, left:16, right:16, backgroundColor:'#1b1731',
